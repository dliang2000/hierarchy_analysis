...
echo java -cp $SOOT_JAR:$JAVA_PATH $CC_CLASS $BENCHMARK_PATH/$TARGET_PATH `cat benchmark_class_path`:$JAR_PATH quartz_core_2.3.1_missing_methods.csv
java -cp $SOOT_JAR:$JAVA_PATH $CC_CLASS $BENCHMARK_PATH/$TARGET_PATH `cat benchmark_class_path`:$JAR_PATH quartz_core_2.3.1_missing_methods.csv
