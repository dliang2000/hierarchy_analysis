...
BENCHMARK_PATH="$MACHINE_SPECIFIC_PATH/Benchmarks/soot-4.0.0-patched"
BENCHMARK="soot_4.0.0"
...
echo java -cp $SOOT_JAR:$JAVA_PATH $CC_CLASS $BENCHMARK_PATH/$TARGET_PATH $jars`cat benchmark_class_path`:$JAR_PATH soot_4.0.0_missing_methods.csv $BENCHMARK
java -cp $SOOT_JAR:$JAVA_PATH $CC_CLASS $BENCHMARK_PATH/$TARGET_PATH $jars`cat benchmark_class_path`:$JAR_PATH soot_4.0.0_missing_methods.csv $BENCHMARK
