...
echo java -cp $SOOT_JAR:$JAVA_PATH $CC_CLASS $BENCHMARK_PATH/$TARGET_PATH $jars`cat benchmark_class_path`:$JAR_PATH jsoup_jsoup_1.10.1_missing_methods.csv $BENCHMARK
java -cp $SOOT_JAR:$JAVA_PATH $CC_CLASS $BENCHMARK_PATH/$TARGET_PATH $jars`cat benchmark_class_path`:$JAR_PATH jsoup_jsoup_1.10.1_missing_methods.csv $BENCHMARK
